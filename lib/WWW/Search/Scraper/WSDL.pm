...
package WWW::Search::Scraper::WSDL;
...
use Carp ();
use WWW::Search::Scraper(qw(2.22));
...
sub new {
    ...
    $self = {};
    bless $self, 'WWW::Search::Scraper::WSDL';
    $self->_wantsNativeRequest($wantsNativeRequest);

    $self->{'agent_name'} = "Mozilla/WWW::Search::Scraper::WSDL/$VERSION";
...
sub init {
    ...
    $self->SetRequest( new WWW::Search::Scraper::Request::WSDL($self, $native_query, $native_options) ) unless ( $self->GetRequest());
    return $self;
}
...
C<WWW::Search::Scraper::WSDL> is written and maintained

...
if ($use eq 'ppm') {
    ...
    install 'WWW::Search' if !eval 'require WWW::Search; WWW::Search->VERSION(2.28)';
...
if (!eval 'require XML::XPath;') { warn $@; $require_errors++ }
...
if (!eval 'require WWW::Search; WWW::Search->VERSION(2.28);') { warn $@; $require_errors++ }
...
if (!eval 'require LWP; LWP->VERSION(5.48);') { warn $@; $require_errors++ }warn "Autoinstallation of prerequisites completed\n" if !$require_errors;
